...
def printCases(input1: list[list[str, str]], act1: list[str,str], input2: list[list[str,str]], act2: list[str, str], duration: int, interval: list[str]) -> None:
    ...
    if (interval[0][1] == ':' and len(interval[0]) == 3) or interval[0][2] == ':' and len(interval[0]) == 4:
        interval[0] += '0'
    if (interval[1][1] == ':' and len(interval[1]) == 3) or interval[1][2] == ':' and len(interval[1]) == 4:
        interval[1] += '0'
...
def greedy(input1: list[list[str, str]], act1: list[str,str], input2: list[list[str,str]], act2: list[str, str], duration: int) -> None:
    ...
    if not input1:
        temp1.append([start,end,end])
    elif timeAddition(start, duration) <= input1[0][0]:
        temp1.append([start, getMaxEnd(start, duration, input1[0][0], end),input1[0][0]])

    if not input2:
        temp2.append([start,end, end])
    elif timeAddition(start, duration) <= input2[0][0]:
        temp2.append([start, getMaxEnd(start, duration, input2[0][0], end), input2[0][0]])

    for i in range(len(input1)-1):
        curr_end1 = input1[i][1]
        nxt_start1 = input1[i+1][0]
        # check if shortest meeting is less than next start time and not in range of previous activites
        shortest_meeting = timeAddition(curr_end1, duration)
        if start <= curr_end1 <= end and shortest_meeting <= nxt_start1:
            temp1.append([curr_end1, getMaxEnd(curr_end1, duration, nxt_start1, end), nxt_start1])
    ...
    for i in range(len(input2)-1):
        ...
        if start <= curr_end2 <= end and shortest_meeting <= nxt_start2:
            temp2.append([curr_end2, getMaxEnd(curr_end2, duration, nxt_start2, end), nxt_start2])


    # consider edge case of last activity end time and add to lists
    if input1 and input1[-1][1] <= end and timeAddition(input1[-1][1], duration) <= end:
        temp1.append([input1[-1][1], getMaxEnd(input1[-1][1], duration, end, end), end])

    if input2 and input2[-1][1] <= end and timeAddition(input2[-1][1], duration) <= end:
        temp2.append([input2[-1][1], getMaxEnd(input2[-1][1], duration, end, end), end])
    ...
    i, res = 0, []
    while i < len(joined):
        curr_start, curr_end, curr_bound = joined[i][0], joined[i][1], joined[i][2]

        if i < len(joined)-1:
            next_start, next_end, next_bound = joined[i+1][0], joined[i+1][1], joined[i+1][2]
        else:
            next_start, next_end, next_bound = None, None, None
        ...
        if next_start and curr_end > next_start:
            ...
            res.append([joined[i][0], joined[i][1]])
            i += 1
...
def runTests() -> None:
    ...
    [[['12:00', '13:30'], ['14:30', '15:00'], ['3:00', '5:00'], ['8:30','10:00']], ['0.00', '23:59'], [['0:20', '4:00'], ['16:12', '19:00']], ['0:00', '23:59'], 150],
    ...
    [[['6:09', '7:39'], ['8:19', '9:52'], ['11:10', '11:37'], ['13:39','14:27'], ['15:31', '16:15']], ['7:39', '17:53'],
[['6:33', '8:20'], ['9:39', '10:20'],['11:13', '12:50'], ['13:51', '14:22'], ['15:22', '15:47'], ['17:05', '17:53']], ['8:00', '18:53'], 47]
